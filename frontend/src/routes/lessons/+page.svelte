<script lang="ts">
  ...
  async function createLesson() {
    ...
    error = '';
    try {
      const lesson = await api.createLesson(token, { title: title.trim() || undefined });
...
<section class="space-y-6">
  <div class="flex flex-wrap items-center justify-between gap-4">
    <div>
      <h1 class="text-2xl">Lessons</h1>
      <p class="text-sm text-slate-500">Grouped note timelines.</p>
    </div>
    <button
      type="button"
