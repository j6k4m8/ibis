<script lang="ts">
  ...
  import type { Lesson, Note, NoteVersion, TranscriptChunk } from '$lib/types';
  ...
  let noteLessons: Lesson[] = [];
  let lessonsLoading = false;
  let lessonsError = '';
  let selectedLessonId = '';
  let addingLesson = false;
  let removingLessonId: string | null = null;
  let createdAtInput = '';
  ...
  function formatLessonTitle(lesson: Lesson): string {
  ...
  async function addLessonToNote() {
    if (!token || !note || !selectedLessonId) {
      return;
    }
    addingLesson = true;
    lessonsError = '';
    try {
      await api.addLessonNote(token, selectedLessonId, note.id);
      noteLessons = await api.listNoteLessons(token, note.id);
      selectedLessonId = '';
    } catch (err) {
      lessonsError = err instanceof Error ? err.message : 'Unable to add lesson.';
    } finally {
      addingLesson = false;
    }
  }

  async function removeLessonFromNote(lessonId: string) {
    if (!token || !note) {
      return;
    }
    removingLessonId = lessonId;
    lessonsError = '';
    try {
      await api.removeLessonNote(token, lessonId, note.id);
      noteLessons = noteLessons.filter((lesson) => lesson.id !== lessonId);
    } catch (err) {
      lessonsError = err instanceof Error ? err.message : 'Unable to remove lesson.';
    } finally {
      removingLessonId = null;
    }
  }
  ...
  async function loadLessonData(activeToken: string, noteId: string) {
    lessonsLoading = true;
    lessonsError = '';
    try {
      const [lessons, noteLessonList] = await Promise.all([
        api.listLessons(activeToken),
        api.listNoteLessons(activeToken, noteId),
      ]);
      allLessons = lessons;
      noteLessons = noteLessonList;
    } catch (err) {
      lessonsError = err instanceof Error ? err.message : 'Unable to load lessons.';
    } finally {
  ...
  $: availableLessons = allLessons.filter(
    (lesson) => !noteLessons.find((item) => item.id === lesson.id),
  );
